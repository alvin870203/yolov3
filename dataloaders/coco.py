"""
Training & validation dataloaders of COCO detection dataset.
"""
...
class CocoTrainDataLoader(DataLoader):
    def __init__(self, config: CocoConfig, img_dir, ann_path, batch_size, num_workers, collate_fn,
        ...
        dataset_train = torchvision.datasets.CocoDetection(root=img_dir, annFile=ann_path, transforms=transforms)
        dataset = wrap_dataset_for_transforms_v2(dataset_train)
        # TODO: whether to _coco_remove_images_without_annotations()
        if nano:
...
class CocoValDataLoader(DataLoader):
    ...
    def __init__(self, config: CocoConfig, img_dir, ann_path, batch_size, num_workers, collate_fn,
        ...
        dataset_val = torchvision.datasets.CocoDetection(root=img_dir, annFile=ann_path, transforms=transforms)
        dataset = wrap_dataset_for_transforms_v2(dataset_val)
        if nano:
            dataset = Subset(dataset, range(5))
        super().__init__(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn,
                         num_workers=num_workers, pin_memory=pin_memory)
...
class BlankCocoTrainDataLoader(DataLoader):
    """All images are set to zeros. Used for setting input-independent baseline."""
    def __init__(self, config: BlankCocoConfig, img_dir, ann_path, batch_size, num_workers, collate_fn,
                 shuffle=True, pin_memory=True,
                 nano=False):  # if True: use only the first five images as the entire dataset
        self.config = config
...
class BlankCocoValDataLoader(DataLoader):
    """All images are set to zeros. Used for setting input-independent baseline."""
    # Default shuffle=True since only eval partial data
    def __init__(self, config: BlankCocoConfig, img_dir, ann_path, batch_size, num_workers, collate_fn,
                 shuffle=True, pin_memory=True,
...
if __name__ == '__main__':
    # Test the dataloaders by `python -m dataloaders.coco` from the workspace directory
    import matplotlib.pyplot as plt
    img_dir_train = 'data/coco/images/train2017'
    ann_path_train = 'data/coco/annotations/instances_train2017.json'
    img_dir_val = 'data/coco/images/val2017'
    ann_path_val = 'data/coco/annotations/instances_val2017.json'
    config = CocoConfig()
    dataloader_train = CocoTrainDataLoader(config, img_dir_train, ann_path_train, batch_size=32, num_workers=4,
                                           collate_fn=coco_collate_fn)
    dataloader_val = CocoValDataLoader(config, img_dir_val, ann_path_val, batch_size=32, shuffle=False, num_workers=4,
                                       collate_fn=coco_collate_fn)
    ...
    print(f"{x.shape=}; {y.shape=}; {border.shape=}")
